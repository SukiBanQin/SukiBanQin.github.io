...
window && (() => {
    let e = {
      flakeCount: window.matchMedia('(max-width: 768px)').matches ? 24 : 50, // 移动端适当减量
      minDist: 150,   // 最小距离
    ...
    const shouldRunSnow = () => !document.hidden
      && localStorage.getItem("snow") === "block"
    ...
    const syncSnowAnimation = () => {
      const shouldRun = shouldRunSnow();
      i.style.display = shouldRun ? "block" : "none";
      if (shouldRun && snowFrameId === null) {
        snowFrameId = t(h);
      } else if (!shouldRun) {
        if (snowFrameId !== null) cancelT(snowFrameId);
        snowFrameId = null;
        n.clearRect(0, 0, i.width, i.height);
      }
    };
    new MutationObserver(syncSnowAnimation).observe(document.documentElement, {
      attributes: true,
...
rmf.syncReadModeMenu = function () {
  const menuItem = document.getElementById('menu-readmode');
  if (!menuItem) return;
  ...
  if (label) label.textContent = isReadMode ? '退出阅读模式' : '阅读模式';
  if (icon) icon.className = isReadMode ? 'fa fa-sign-out-alt' : 'fa fa-book';
}

rmf.exitReadMode = function () {
  document.body.classList.remove('read-mode');
  document.querySelectorAll('.exit-readmode').forEach(button => button.remove());
  rmf.syncReadModeMenu();
}
  ...
  window.oncontextmenu = function (event) {
    ...
    if (el.tagName == 'A') {
      ...
      rmf.openWithNewTab = function () {
        ...
        imageWindow.document.title = el.alt || '图片预览';
        imageWindow.document.documentElement.style.background = '#111';
        imageWindow.document.body.style.cssText = 'margin:0;min-height:100vh;display:flex;align-items:center;justify-content:center;background:#111;';
        const preview = imageWindow.document.createElement('img');
        preview.src = imageUrl;
        preview.alt = el.alt || '';
        preview.style.cssText = 'display:block;max-width:100%;max-height:100vh;object-fit:contain;';
        imageWindow.document.body.appendChild(preview);
      }
      rmf.click = function () {
        el.click()
      }
      ...
      rmf.saveAs = function () {
        ...
        let filename = 'image';
        try {
          filename = decodeURIComponent(new URL(url, window.location.href).pathname.split('/').pop()) || filename;
        } catch (error) { }
        a.href = url;
        a.download = filename;
...
function createtime() {
  ...
  var start = new Date("01/25/2022 00:00:00"); // 旅行者1号开始计算的时间
  var dis = Math.trunc(23400000000 + ((now - start) / 1000) * 17); // 距离=秒数*速度 记住转换毫秒
  var unit = (dis / 149600000).toFixed(6);  // 天文单位
  // 网站诞生时间
  var grt = new Date("01/25/2022 00:00:00");
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<img class='boardsign' src='https://img.shields.io/badge/坂琴的窝-营业中-6adea8?style=social&logo=cakephp' title='今天也要努力学习哦~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`
      : `<img class='boardsign' src='https://img.shields.io/badge/坂琴的窝-下班了该睡觉了~-6adea8?style=social&logo=coffeescript' title='该睡觉了（￣︶￣）↗　'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
...
/* fps检测 start */
var fpsFrameId = null;
var fpsMonitorEnabled = false;
var fpsFrameCount = 0;
var fpsLastSampleTime = Date.now();

function fpsStatusText(fps) {
...
function fpsLoop() {
  if (!fpsMonitorEnabled || document.hidden) {
    fpsFrameId = null;
    return;
  }
  ...
  const now = Date.now();
  fpsFrameCount++;
  if (now - fpsLastSampleTime >= 1000) {
    const fps = Math.round((fpsFrameCount * 1000) / (now - fpsLastSampleTime));
    const target = document.getElementById("fps");
    if (target) target.innerHTML = `FPS:${fps} ${fpsStatusText(fps)}`;
    fpsFrameCount = 0;
    fpsLastSampleTime = now;
  }
  fpsFrameId = window.requestAnimationFrame(fpsLoop);
}
...
function startFpsMonitor() {
  ...
  fpsFrameCount = 0;
  fpsLastSampleTime = Date.now();
  if (!document.hidden && fpsFrameId === null) fpsFrameId = window.requestAnimationFrame(fpsLoop);
}

function stopFpsMonitor() {
  fpsMonitorEnabled = false;
  if (fpsFrameId !== null) window.cancelAnimationFrame(fpsFrameId);
  fpsFrameId = null;
  const target = document.getElementById("fps");
  if (target) {
...
document.addEventListener("visibilitychange", function () {
  if (document.hidden && fpsFrameId !== null) {
    window.cancelAnimationFrame(fpsFrameId);
    fpsFrameId = null;
  } else if (!document.hidden && fpsMonitorEnabled && fpsFrameId === null) {
    fpsFrameCount = 0;
    fpsLastSampleTime = Date.now();
    fpsFrameId = window.requestAnimationFrame(fpsLoop);
  }
...
window.localStorage.getItem("fpson") == "1" ? startFpsMonitor() : stopFpsMonitor();
/* fps检测 end */
...
function clearItem() {
  ...
  if (typeof stopFpsMonitor === 'function') stopFpsMonitor();
}
...
if (localStorage.getItem("font") == undefined) {
  ...
  localStorage.setItem("font", "YZSOFT");
}
...
function setFontBorder() {
  ...
  var currentFontButton = document.getElementById(swfId);
  if (!currentFontButton) return;
  currentFontButton.style.border = "2px solid var(--theme-color)";
...
if (localStorage.getItem("universe") == undefined) {
  localStorage.setItem("universe", "block");  //如果想默认开就改成block,默认关就是none
}
...
if (localStorage.getItem("snow") == undefined) {
  localStorage.setItem("snow", "none");   //如果想默认开就改成block,默认关就是none
}
...
function fpssw() {
  const enabled = document.getElementById("fpson").checked;
  localStorage.setItem("fpson", enabled ? "1" : "0");
  enabled ? startFpsMonitor() : stopFpsMonitor();
}
...
var curTransMini = curTransNum;
function applyTransparency(value) {
  const percentage = Math.max(0, Math.min(100, Number(value) || 0));
  const alpha = percentage / 100;
  const lightValue = `rgba(253, 253, 253, ${alpha})`;
  const darkValue = `rgba(25, 25, 25, ${alpha})`;
  document.documentElement.style.setProperty('--trans-light', lightValue, 'important');
  document.documentElement.style.setProperty('--trans-dark', darkValue, 'important');
  const style = document.getElementById("transPercent");
  if (style) style.textContent = `:root{--trans-light:${lightValue} !important;--trans-dark:${darkValue} !important;}`;
}
applyTransparency(curTransNum);
function setTrans() {
  var elem = document.getElementById("transSet");
  var newTransNum = elem.value;
  var target = document.querySelector('.transValue');
  if (target) target.textContent = "透明度 (0%-100%): " + newTransNum + "%";
  localStorage.setItem("transNum", newTransNum);
  curTransMini = newTransNum;
  curTransNum = newTransNum;  // 更新当前透明度
  const range = document.querySelector('#rang_trans');
  if (range) range.style.width = curTransMini + "%";
  applyTransparency(newTransNum);
};
...
var miniBlur = curBlur;
function applyBlurRadius(value) {
  const radius = Math.max(0, Math.min(100, Number(value) || 0));
  const blurValue = `blur(${radius}px) saturate(120%)`;
  document.documentElement.style.setProperty('--blur-num', blurValue, 'important');
  const style = document.getElementById("blurNum");
  if (style) style.textContent = `:root{--blur-num:${blurValue} !important;}`;
}
applyBlurRadius(curBlur);
function setBlurNum() {
  var elem = document.getElementById("blurSet");
  var newBlur = elem.value;
  var target = document.querySelector('.blurValue');
  if (target) target.textContent = "模糊半径 (开启模糊生效 0px-100px): " + newBlur + "px";
  localStorage.setItem("blurRad", newBlur);
  curBlur = newBlur;
  miniBlur = curBlur;
  const range = document.querySelector('#rang_blur');
  if (range) range.style.width = miniBlur + "%";
  applyBlurRadius(curBlur);
};
...
function applyBlurState(enabled) {
  const filterValue = enabled ? 'var(--blur-num)' : 'none';
  document.documentElement.style.setProperty('--backdrop-filter', filterValue, 'important');
  const style = document.getElementById("settingStyle");
  if (style) style.textContent = `:root{--backdrop-filter:${filterValue} !important;}`;
}
applyBlurState(localStorage.getItem("blur") == 1);
function setBlur() {
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  const style = document.getElementById("defineBg");
...
function normalizeBackgroundValue(value) {
  ...
  if (/^url\((.|\s)*\)$/i.test(input)) return input;
  try {
...
// 切换背景主函数
function changeBg(s) {
  const normalized = normalizeBackgroundValue(s);
  if (!normalized) return false;
  // 自定义颜色框
  defineColor = normalized.charAt(0) == "#" ? normalized : '#F4D88A';
  setBg(normalized);
  localStorage.setItem("blogbg", normalized);
  return true;
}
// 设置背景属性
function setBg(s) {
  ...
  const backgroundValue = normalized.charAt(0) === '#'
    ? normalized
    : `${normalized} center / cover no-repeat`;
  const style = document.getElementById("defineBg");
  if (style) {
    style.textContent = `#web_bg{background:${backgroundValue} !important;background-attachment:local !important;}`;
  }
  const background = document.getElementById("web_bg");
  if (background) {
    background.style.setProperty('background', backgroundValue, 'important');
    background.style.setProperty('background-attachment', 'local', 'important');
  }
}

document.addEventListener('pjax:complete', function () {
  const savedBackground = localStorage.getItem('blogbg');
...
function getPicture_() {
  const input = document.getElementById("pic-link");
  const pictureUrl = input ? input.value.trim() : '';
  let parsedUrl = null;
  try {
    parsedUrl = new URL(pictureUrl, window.location.href);
  } catch (error) { }

  if (!parsedUrl || !['http:', 'https:'].includes(parsedUrl.protocol)) {
    banqinNotify({
      title: "链接不对🤣",
      message: "请输入以 http:// 或 https:// 开头的有效图片链接！",
      type: "warning",
      duration: 5000
    });
    return;
  ...
  checkImgExists(parsedUrl.href).then(() => {
    // 有效的图片链接
    changeBg(parsedUrl.href);
    // 提示切换成功
    banqinNotify({
      title: "可以啦🍨",
      message: "切换自定义背景成功！",
...
<h2>一、显示偏好</h2>
<p class="settings-help">透明度作用于首页卡片、侧栏和正文容器；模糊半径需要同时开启“模糊效果”后才会显示。</p>
...
<div class="settings-toggle-grid">
  <label class="settings-toggle"><span class="content-text" style="font-weight:bold">星空特效 (夜间模式)</span><input type="checkbox" id="universeSet" onchange="setUniverse()"></label>
  <label class="settings-toggle"><span class="content-text" style="font-weight:bold">霓虹灯 (夜间模式)</span><input type="checkbox" id="lightSet" onchange="setLight()"></label>
  <label class="settings-toggle"><span class="content-text" style="font-weight:bold">模糊效果 (消耗性能)</span><input type="checkbox" id="blur" onchange="setBlur()"></label>
  <label class="settings-toggle"><span class="content-text" style="font-weight:bold">侧边栏 (默认开)</span><input type="checkbox" id="rightSideSet" onchange="toggleRightside()"></label>
  <label class="settings-toggle"><span class="content-text" style="font-weight:bold">帧率监测</span><input type="checkbox" id="fpson" onchange="fpssw()"></label>
  <label class="settings-toggle"><span class="content-text" style="font-weight:bold">雪花特效 (白天模式)</span><input type="checkbox" id="snowSet" onchange="setSnow()"></label>
</div>
...
<h3>1. 画廊图片</h3>
<details class="folding-tag" cyan><summary> 查看画廊图片 </summary>
              <div class='content'>
              <div class="bgbox"><a href="/box/gallery" rel="noopener external nofollow" style="background-image:url(https://coscdn.ibanqin.com/images/coverimages/2025/2025-5-10/129971794_p0.webp)" class="imgbox" onclick="changeBg('url(https://coscdn.ibanqin.com/images/coverimages/2025/2025-5-10/129971794_p0.webp)')"></a><a href="/box/gallery" rel="noopener external nofollow" style="background-image:url(https://coscdn.ibanqin.com/images/coverimages/2025/5-14/store.webp)" class="imgbox" onclick="changeBg('url(https://coscdn.ibanqin.com/images/coverimages/2025/5-14/store.webp)')"></a><a href="/box/gallery" rel="noopener external nofollow" style="background-image:url(https://coscdn.ibanqin.com/images/coverimages/2025/5-14/ailisi.webp)" class="imgbox" onclick="changeBg('url(https://coscdn.ibanqin.com/images/coverimages/2025/5-14/ailisi.webp)')"></a><a href="/box/gallery" rel="noopener external nofollow" style="background-image:url(https://coscdn.ibanqin.com/images/coverimages/2025/5-14/yewan_jiaotang.webp)" class="imgbox" onclick="changeBg('url(https://coscdn.ibanqin.com/images/coverimages/2025/5-14/yewan_jiaotang.webp)')"></a><a href="/box/gallery" rel="noopener external nofollow" style="background-image:url(https://coscdn.ibanqin.com/images/coverimages/2025/5-14/summer_night.webp)" class="imgbox" onclick="changeBg('url(https://coscdn.ibanqin.com/images/coverimages/2025/5-14/summer_night.webp)')"></a></div>
              </div>
            </details>
...
<details class="folding-tag" cyan><summary> 设置自定义背景 </summary>
              <div class='content'>
              <p class="settings-help">填写直链后会立即替换全站背景，并同时应用到电脑/手机与日间/夜间模式。</p><p><center><input type="text" id="pic-link" size="70%" maxlength="1000" placeholder="请输入有效的图片链接，如 https://coscdn.ibanqin.com/images/coverimages/2025/2025-5-11/104088864_p0.webp"></center></p><p><center><button type="button" onclick="getPicture()" style="background:var(--theme-color);width:35%;padding: 5px 0px 7px 0px;border-radius:30px;color:white;line-height:2;">🌈切换背景🌈</button></center></p>
              </div>
            </details>
  ...
  winboxLoadPromise = new Promise((resolve, reject) => {
    const loadSource = (index) => {
      if (index >= sources.length) {
        reject(new Error("WinBox failed to load"));
        return;
      }
      ...
      script.async = true;
      script.onload = () => typeof WinBox === "function" ? resolve() : loadSource(index + 1);
      script.onerror = () => {
        script.remove();
        loadSource(index + 1);
      };
